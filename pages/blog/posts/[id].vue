...
const deleteImage = () => {
  modal.open(CmsConfirmModal, {
    ...
    message:
      "Вы уверены что хотите удалить изображение? После выполнения этого действия, для восстановления изображения, потребуется загрузить его вновь!",
    onClose: () => {
      toast.add({
        color: "red",
    ...
    onResolve: async () => {
      toast.add({
        color: "green",
        title: "Вы успешно удалили изображение!",
      });
